...
async def handle(user_id: str, text: str, session: Dict[str, Any]) -> Dict[str, str]:
    ...
    if step == "await_items":
        data["items"] = text
        await db.sessions.update_one({"user_id": user_id}, {"$set": {"step": "await_address", "data": data, "updated_at": datetime.utcnow()}})
        await send_message(user_id, "Please provide your delivery address.")
        return {"status": "awaiting"}

    if step == "await_address":
        data["address"] = text
        await db.sessions.update_one({"user_id": user_id}, {"$set": {"step": "await_confirm", "data": data, "updated_at": datetime.utcnow()}})
    ...
    if step == "await_confirm":
        if text.strip().lower().startswith("y"):
            order = {"user_id": user_id, "items": data.get("items"), "address": data.get("address"), "created_at": datetime.utcnow()}
